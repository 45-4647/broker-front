...
export default function ChatList({ theme = "dark" }) {
  ...
  useEffect(() => {
    ...
    if (storedUser) {
      ...
      setUserId(parsed.id); // make sure backend also uses "id" not "_id"
    }
  }, []);
  ...
  return (
    ...
    >
      <div className="w-full max-w-2xl">
        ...
        {/* Chat List */}
        <div
          className={`w-full border rounded-3xl shadow-2xl p-5 overflow-y-auto backdrop-blur-xl transform transition-all duration-500 hover:-translate-y-1 ${
            isDark
              ...
              {rooms.map((room) => {
                ...
                return (
                  <div key={room._id} className="relative">
                    <Link
                      to={`/chat/${room._id}`}
                      className={`block p-3 rounded-2xl border shadow-sm relative transition-all duration-200 ${
                        isDark
