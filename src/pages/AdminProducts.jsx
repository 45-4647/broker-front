...
const ROLE_COLORS = { admin: "bg-purple-500/10 text-purple-400 border-purple-500/20", seller: "bg-blue-500/10 text-blue-400 border-blue-500/20", buyer: "bg-slate-700/30 text-slate-400 border-slate-600/30" };
const NAV = [{ id: "overview", label: "Overview", icon: "📊" }, { id: "products", label: "Products", icon: "📦" }, { id: "users", label: "Users", icon: "👥" }, { id: "sellers", label: "Sellers", icon: "🏪" }, { id: "reports", label: "Reports", icon: "📋" }];
...
export default function AdminProducts({ theme = "dark" }) {
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      ...
      const [pRes, uRes] = await Promise.all([API.get("/admin/products", { headers: h }), API.get("/admin/users", { headers: h })]);
      setProducts(pRes.data); setUsers(uRes.data);
  ...
  const del = async (url, setter, id) => {
    setDeletingId(id);
    const tid = toast.loading("Deleting...");
    try {
      await API.delete(url, { headers: { Authorization: Bearer  } });
  ...
  const handleRoleChange = async (userId, newRole) => {
    ...
    try {
      const res = await API.put(/admin/users//role, { role: newRole }, { headers: { Authorization: Bearer  } });
      setUsers(u => u.map(x => x._id === userId ? res.data : x));
      toast.success(Role: , { id: tid });
    } catch { toast.error("Failed.", { id: tid }); }
  ...
  const ConfirmDelete = ({ onConfirm, id }) => (
    <div className="flex gap-1.5">
      <button onClick={onConfirm} disabled={deletingId === id} className="flex-1 py-1.5 rounded-xl bg-red-500 hover:bg-red-600 text-white text-xs font-semibold transition flex items-center justify-center">
        {deletingId === id ? <svg className="w-3 h-3 animate-spin" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8z"/></svg> : "Confirm"}
      </button>
  ...
  return (
    <div className="bg-slate-950 text-slate-100 min-h-screen flex">
      ...
      {/* SIDEBAR */}
      <aside className="w-64 shrink-0 border-r border-slate-800 bg-slate-900 flex flex-col hidden lg:flex">
        ...
        <main className="flex-1 overflow-y-auto p-6">
          ...
          {activeNav === "overview" && (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                <StatCard label="Sellers" value={sellers.length} sub="Active sellers" icon="🏪" accent="#3b82f6" />
                <StatCard label="Buyers" value={buyers.length} sub="Registered buyers" icon="🛒" accent="#06b6d4" />
                <StatCard label="Pending Listings" value={products.filter(p => p.paymentStatus !== "paid").length} sub="Unpaid" icon="⏳" accent="#f59e0b" />
                <StatCard label="Admins" value={users.filter(u => u.role === "admin").length} sub="Admin accounts" icon="🛡️" accent="#8b5cf6" />
              </div>
                  ...
                  {[{ role: "buyer", count: buyers.length, color: "bg-slate-500" }, { role: "seller", count: sellers.length, color: "bg-blue-500" }, { role: "admin", count: users.filter(u => u.role === "admin").length, color: "bg-purple-500" }].map(r => (
                    <div key={r.role} className="flex items-center gap-3">
                      ...
                      <div className="flex-1 h-2 bg-slate-800 rounded-full overflow-hidden">
                        <div className={h-full  rounded-full transition-all duration-700} style={{ width: users.length ? ${(r.count / users.length) * 100}% : "0%" }} />
                      </div>
                  ...
                  {products.slice(0, 5).map(p => (
                    <div key={p._id} className="flex items-center gap-3 p-3 rounded-xl bg-slate-800">
                      ...
                      <div className="text-right shrink-0">
                        ...
                        <span className={	ext-[10px] px-1.5 py-0.5 rounded-full font-medium }>{p.paymentStatus || "pending"}</span>
                      </div>
                    </div>
                  ))}
                </div>
          ...
          {activeNav === "products" && (
            <div>
              <div className="flex items-center justify-between mb-5">
                <p className="text-sm text-slate-400">{fp.length} product{fp.length !== 1 ? "s" : ""} found</p>
              </div>
              {fp.length === 0 ? (
                    ...
                    <div key={p._id} className={${card} flex flex-col overflow-hidden hover:-translate-y-0.5 transition-transform}>
                      ...
                      <div className="p-4 flex flex-col flex-1">
                        <div className="flex items-start justify-between gap-2 mb-1">
                          ...
                          <span className={shrink-0 text-[10px] px-2 py-0.5 rounded-full font-medium border }>{p.paymentStatus || "pending"}</span>
                        </div>
                        ...
                        <div className="mt-auto">
                          {confirmingId === p._id
                            ? <ConfirmDelete onConfirm={() => del(/admin/products/, setProducts, p._id)} id={p._id} />
              ...
              {fu.map(u => (
                <div key={u._id} className={${card} flex flex-col sm:flex-row sm:items-center gap-4 p-4}>
                  <div className="w-11 h-11 rounded-full bg-gradient-to-br from-blue-500 to-indigo-500 flex items-center justify-center text-white font-bold text-lg shrink-0">
                    {u.profileImage ? <img src={u.profileImage} alt="" className="w-11 h-11 rounded-full object-cover" /> : u.name?.charAt(0).toUpperCase()}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-wrap items-center gap-2 mb-0.5">
                      <p className="font-semibold text-sm text-white">{u.name}</p>
                      <span className={	ext-xs px-2 py-0.5 rounded-full border font-medium }>{u.role}</span>
                  ...
                  {u._id !== adminUser?.id && (
                    confirmingId === u._id
                      ? <div className="flex gap-2 shrink-0"><ConfirmDelete onConfirm={() => del(/admin/users/, setUsers, u._id)} id={u._id} /><button onClick={() => setConfirmingId(null)} className="px-3 py-1.5 rounded-xl text-xs border border-slate-700 hover:bg-slate-800 transition">Cancel</button></div>
          ...
          {activeNav === "sellers" && (
            <div className="space-y-4">
              ...
              {fs.map(s => {
                const sellerProducts = products.filter(p => p.seller?._id === s._id || p.seller === s._id);
                const sellerRevenue = Math.round(sellerProducts.filter(p => p.paymentStatus === "paid").reduce((sum, p) => sum + parseFloat(p.price) * 0.03, 0));
                return (
                  <div key={s._id} className={${card} p-5}>
                    <div className="flex items-start gap-4">
                      <div className="w-12 h-12 rounded-full bg-gradient-to-br from-blue-500 to-indigo-500 flex items-center justify-center text-white font-bold text-lg shrink-0">
                        {s.profileImage ? <img src={s.profileImage} alt="" className="w-12 h-12 rounded-full object-cover" /> : s.name?.charAt(0).toUpperCase()}
                      </div>
                      <div className="flex-1 min-w-0">
                      ...
                      <div className="grid grid-cols-2 gap-3 shrink-0">
                        <div className="text-center bg-slate-800 rounded-xl px-3 py-2">
                          <p className="text-lg font-extrabold text-white">{sellerProducts.length}</p>
                          <p className="text-[10px] text-slate-400">Listings</p>
                        </div>
                        <div className="text-center bg-slate-800 rounded-xl px-3 py-2">
                          <p className="text-sm font-extrabold text-amber-400">{sellerRevenue.toLocaleString()}</p>
                    ...
                    {sellerProducts.length > 0 && (
                      <div className="mt-4 flex gap-2 overflow-x-auto pb-1">
                        {sellerProducts.slice(0, 4).map(p => (
                          <Link key={p._id} to={/product/} className="shrink-0 w-20 rounded-xl overflow-hidden border border-slate-700 hover:border-blue-500 transition">
                    ...
                    <div className="flex gap-2 mt-4">
                      ...
                      {confirmingId === s._id
                        ? <><ConfirmDelete onConfirm={() => del(/admin/users/, setUsers, s._id)} id={s._id} /><button onClick={() => setConfirmingId(null)} className="px-3 py-1.5 rounded-xl text-xs border border-slate-700 hover:bg-slate-800 transition">Cancel</button></>
            ...
            <div className="space-y-5">
                ...
                <div className={${card} p-6}>
                  ...
                  <div className="space-y-3">
                    {[
                      ...
                      { label: "Paid / Active", value: ${paidProducts.length} /  },
              ...
              <div className={${card} p-6}>
                ...
                <div className="grid sm:grid-cols-2 gap-3">
                  {[
                    { icon: "👥", title: "User Management", desc: "Add, remove, or change roles of any user. Promote buyers to sellers or demote sellers." },
                    { icon: "📦", title: "Product Moderation", desc: "Review and remove inappropriate or fraudulent listings from the marketplace." },
